...
"""
    ...
    if request.current_module == "模块一：史实认知":
        ...
        system_prompt += """
        你正处于【模块四：反思总结】。
        你的**首要任务**是引导用户完成他们正在撰写的**总结卡片**。
        """

        # 动态地将卡片标题（如果存在）注入到 Prompt 中
        # (这依赖于您在 main_schemas.py 中添加的字段)
        if request.card_title:
            system_prompt += f"""

            **【当前总结卡片标题】: "{request.card_title}"**
            
            你的所有提问都必须**紧密围绕**这个卡片标题，帮助学生深化对这个特定问题的思考。
            
            当用户（通过点击提示按钮）询问关于某个具体方面（例如“各方动机与角色解读”）时，你的引导性问题必须**同时关联**这个“提示”和“{request.card_title}”这个最终目标。

            **引导示例（严格遵守此模式）：**
            - 用户提问：“针对‘各方动机与角色解读’，我该怎么想？”
            - 你的提问（应类似）：“很好。为了帮助你形成‘{request.card_title}’的看法，你认为在‘各方动机与角色解读’中，哪一方的动机对历史走向的影响最为关键？”
            - 你的提问（应类似）：“要形成‘{request.card_title}’，你觉得在‘各方动机’方面，我们最容易忽略掉哪个群体的视角？”

            ---
            （在上述核心任务基础上，你依然可以融合以下通用原则）：
            """
        else:
             # 如果前端没有提供 card_title，回退到原来的通用逻辑
             system_prompt += """
